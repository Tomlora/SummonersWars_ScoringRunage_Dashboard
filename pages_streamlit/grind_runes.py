...
def optimisation_rune(category_selected, coef_set):
    ...
    data_mobs = pd.DataFrame.from_dict(st.session_state['data_json'], orient="index").transpose()
    ...
    data_short = data[['rune_set', 'rune_slot', 'rune_equiped', 'efficiency', 'efficiency_max_hero', 'efficiency_max_lgd', 'potentiel_max_lgd', 'potentiel_max_hero', 'Commentaires', 'Grind_lgd', 'Grind_hero']]
    ...
    with st.spinner('Ajout des informations pour identifier les runes...'):
        ...
        # Graphique
        fig_hero_manquante = px.histogram(df_count, x='Set', y='Meules (hero) manquantes pour la stat max', color='Propriété Meules', title="Meules heroiques manquantes pour la stat max", text_auto=True)


        fig_lgd_manquante = px.histogram(df_count, x='Set', y='Gemmes (hero) manquantes', color='Propriété Gemmes', title="Gemmes heroiques manquantes pour la stat max", text_auto=True)
        ...
        df_inventaire = pd.DataFrame.from_dict(st.session_state['data_json'], orient='index').transpose()
    ...
    def charge_data(data, data_short, df_rune, df_count, df_inventaire):
        with st.spinner('Chargement des données concaténées...Prévoir quelques secondes'):
            ...
            data_xlsx = export_excel(data, data_short, df_rune, df_count, df_inventaire)
            
            # Mise en forme : 
            
            data['rune_equiped'] = data['rune_equiped'].astype('str')
            data_short['rune_equiped'] = data['rune_equiped'].astype('str')
            data_short['rune_equiped'] = data_short['rune_equiped'].replace({'0' : 'Inventaire'})
    ...
    data_xlsx, data, data_short, df_rune, df_count, df_inventaire = charge_data(data, data_short, df_rune, df_count, df_inventaire)    
    ...
    st.download_button('Télécharger la data (Excel)', data_xlsx, file_name='grind.xlsx', mime='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
