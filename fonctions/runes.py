...
class Rune():
    def __init__(self, data_json):
        ...
        self.data['first_sub_value_total'] = (self.data['first_sub_value'] + self.data['first_sub_grinded_value'])
        self.data['second_sub_value_total'] = (self.data['second_sub_value'] + self.data['second_sub_grinded_value'])
        self.data['third_sub_value_total'] = (self.data['third_sub_value'] + self.data['third_sub_grinded_value'])
        self.data['fourth_sub_value_total'] = (self.data['fourth_sub_value'] + self.data['fourth_sub_grinded_value'])
    ...
    def scoring_rune(self, category_selected, coef_set):
        ...
        self.value_selected = result[result['rune_set'].isin(category_selected)]
        ...
        for set in category_selected:
            self.value_selected['points'] = np.where(self.value_selected['rune_set'] == set, self.value_selected['points'] * coef_set[set], self.value_selected['points'])
            
            
            
        self.value_autres = self.value_autres.groupby('efficiency_binned').sum()
        ...
        self.tcd_value = df_value.pivot_table(df_value, 'rune_set', 'efficiency_binned', 'sum')['efficiency']
    ...
    def scoring_spd(self, category_selected_spd, coef_set_spd):
        ...
        self.data_spd['spd_binned'] = pd.cut(self.data_spd['spd'],bins=(23, 26, 29, 32, 36, 40), right=False)

        self.data_spd.dropna(inplace=True)

        self.result_spd = self.data_spd.groupby(['rune_set', 'spd_binned']).count()
        ...
        for key, value in palier_spd.items():
            self.result_spd['factor_spd'] = np.where(self.result_spd['spd_binned'] == key, value, self.result_spd['factor_spd'])
    
        self.result_spd['points_spd'] = self.result_spd['spd'] * self.result_spd['factor_spd']

        # on sépare les dataset à mettre en évidence et les autres

        self.value_selected_spd = self.result_spd[self.result_spd['rune_set'].isin(category_selected_spd)]
        self.value_autres_spd = self.result_spd[~self.result_spd['rune_set'].isin(category_selected_spd)]

        self.value_selected_spd.drop(['factor_spd'], axis=1, inplace=True)

        for set in category_selected_spd:
            self.value_selected_spd['points_spd'] = np.where(self.value_selected_spd['rune_set'] == set, self.value_selected_spd['points_spd'] * coef_set_spd[set], self.value_selected_spd['points_spd'])

        self.value_autres_spd = self.value_autres_spd.groupby('spd_binned').sum()
        ...
        self.df_value_spd = pd.concat([self.value_selected_spd, self.value_autres_spd])
        ...
        self.tcd_value_spd = self.df_value_spd.pivot_table(self.df_value_spd, 'rune_set', 'spd_binned', 'sum')['spd']
        ...
        self.tcd_value_spd .loc['Total'] = [total_23_spd, total_26_spd, total_29_spd, total_32_spd, total_36_spd]
